export type DefaultPurchaseFormField = {
  key: string;
  label: string;
  type: string;
  section: string;
...
export type DefaultPurchaseForm = {
  name: string;
  fields: DefaultPurchaseFormField[];
};
...
function field(
  ...
  type: string,
  section: string,
  extra?: Partial<DefaultPurchaseFormField>
): DefaultPurchaseFormField {
  return { key, label, type, section, isRequired: false, ...extra };
}

export const defaultPurchaseFormsBySlug: Record<string, DefaultPurchaseForm> = {
  "motor-insurance": {
